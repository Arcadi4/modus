...
const MANAGED_MARKER = "<!-- MANAGED BY MODUS -->"
...
describe("syncFiles managed lifecycle", () => {
  ...
  it("creates managed agent files from descriptors in a fresh target", async () => {
    ...
    const descriptors = [descriptor("alpha", "alpha-hash")]
    ...
    expect(result.files).toContainEqual(
      expect.objectContaining({ path: agentPath(targetDir, "alpha"), operation: "created" })
    )
    const contents = await readUtf8(agentPath(targetDir, "alpha"))
  ...
  it("skips unchanged managed files on rerun without rewriting them", async () => {
    ...
    const filePath = agentPath(targetDir, "alpha")
  ...
  it("updates a managed file when descriptor content or hash changes", async () => {
    const { target, targetDir } = await makeTarget()
    await syncWithDescriptors(target, [descriptor("alpha", "alpha-v1", "first description")])

    const result = await syncWithDescriptors(target, [
      descriptor("alpha", "alpha-v2", "second description"),
    ])
  ...
  it("deletes stale generated files when their descriptor disappears", async () => {
    ...
    await syncWithDescriptors(target, [
      descriptor("alpha", "alpha-hash"),
      descriptor("beta", "beta-hash"),
    ])

    const result = await syncWithDescriptors(target, [descriptor("alpha", "alpha-hash")], {
    ...
    expect(result.files).toContainEqual(
      expect.objectContaining({ path: agentPath(targetDir, "beta"), operation: "deleted" })
    )
    await expect(fileExists(agentPath(targetDir, "alpha"))).resolves.toBe(true)
    await expect(fileExists(agentPath(targetDir, "beta"))).resolves.toBe(false)
  })
  ...
  it("refuses to overwrite unmanaged files at target paths by default", async () => {
    ...
    expect(result.files).toContainEqual(
      expect.objectContaining({
        path: filePath,
        operation: "refused",
        reason: expect.stringMatching(/unmanaged|collision/i),
      })
    )
  ...
  it("reports dry-run operations without mutating the filesystem", async () => {
    ...
    await expect(fileExists(agentPath(targetDir, "alpha"))).resolves.toBe(false)
  })
  ...
  it("refuses managed updates when no-overwrite policy is requested", async () => {
    const { target, targetDir } = await makeTarget()
    const filePath = agentPath(targetDir, "alpha")
    await syncWithDescriptors(target, [descriptor("alpha", "alpha-v1")])
    const originalContents = await readUtf8(filePath)

    const result = await syncWithDescriptors(target, [descriptor("alpha", "alpha-v2")], {
      backup: false,
      overwritePolicy: "error",
    })
  ...
  it("refuses corrupted managed files with malformed metadata instead of overwriting them", async () => {
    ...
    expect(result.files).toContainEqual(
      expect.objectContaining({
        path: filePath,
        operation: "refused",
        reason: expect.stringMatching(/malformed|corrupt/i),
      })
    )
  ...
  it("keeps the previous managed file intact if an atomic replacement write fails", async () => {
    const { target, targetDir } = await makeTarget()
    const filePath = agentPath(targetDir, "alpha")
    await syncWithDescriptors(target, [descriptor("alpha", "alpha-v1", "stable description")])
    const originalContents = await readUtf8(filePath)

    const result = await syncWithDescriptors(
      target,
      [descriptor("alpha", "alpha-v2", "new description")],
      {
